    ...
    <div>
        ...
        <table class="table table-hover" v-if="cart.length">
            <caption class="text-right h3">
                <b>Total: </b>
                <price class="d-block text-success font-weight-light" :value="Number(cartTotal)" :precision="3"></price>
            </caption>
            <thead>
                <tr>
                    <th scope="col"></th>
            ...
            <tbody>
                <tr v-for="(item, index) in cart" :key="item.product.id">
                    ...
                    <th class="text-right">{{ Number(item.product.price).toFixed(3) }}</th>
                    <th class="text-right">{{ Number(item.product.price * item.qty).toFixed(3) }}</th>
                </tr>
            </tbody>
        </table>
